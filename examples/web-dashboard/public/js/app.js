  ...
  try {
    ...
    if (!response.ok || !data.success) {
      ...
      if (response.status === 404 || data.error?.includes('not found')) {
        ...
        showStatus(
          'auth-status',
          'Community not found. Please create a new community or login with a valid ID.',
          'error'
        );
        return;
      }
...
function renderPlatformCheckboxes() {
  ...
  if (!token) {
    container.innerHTML =
      '<p class="info-text">Please login to connect platforms and create streams.</p>';
    return;
  }
  ...
  fetch('/api/v1/platforms', {
    ...
    .then((data) => {
      if (data.success && data.data.platforms && data.data.platforms.length > 0) {
        container.innerHTML = data.data.platforms
          .map((p) => {
...
function renderStreams() {
  ...
  if (streams.length === 0) {
    container.innerHTML =
      '<p class="info-text">No streams yet. Create your first stream above!</p>';
    return;
  }
  ...
  container.innerHTML = streams
    .map((stream) => {
      ...
      const platformTags = (stream.platforms || [])
        .map((p) => `<span class="stream-platform-tag">${p}</span>`)
        .join('');

      // Get platform statuses
      const platformStatusHtml =
        platformStreams.length > 0
          ? `
...
async function createStream() {
  ...
  const selectedPlatforms = Array.from(document.querySelectorAll('.platform-checkbox:checked')).map(
    (cb) => cb.value
  );
      ...
      streams = streams.map((s) =>
        s.id === streamId
          ? {
              ...
              ...(updated.stream || {}),
              platformStreams:
                updated.platformStreams ||
                (s.platformStreams || []).map((ps) => ({
                  ...ps,
                  status: 'ended',
                })),
            }
...
function displayScheduledEvents(jobs) {
  ...
  if (jobs.length === 0) {
    listContainer.innerHTML =
      '<p class="text-muted" id="no-events-message">No scheduled events</p>';
    return;
  }
  ...
  listContainer.innerHTML = jobs
    .map((job) => {
      ...
      const platformBadges = job.platforms
        .map((p) => {
          ...
          if (job.status === 'done' || job.status === 'failed') {
            if (result) {
              if (result.status === 'posted' || result.status === 'scheduled') {
                badgeColor = '#e8f5e9';
                statusIcon = '✓';
                statusText = result.postUrl
                  ? `<a href="${escapeHtml(result.postUrl)}" target="_blank" style="color: #2e7d32; text-decoration: none; font-size: 11px;">View →</a>`
                  : '';
              } else if (result.status === 'failed') {
                badgeColor = '#ffebee';
                statusIcon = '✗';
